...
**События (Events) в Битрикс** - это мощный механизм для расширения функциональности без изменения ядра системы.

## **1. Что такое события и зачем они нужны**

**Событие** - это точка в коде Битрикс, где система "сообщает" о выполнении определенного действия, и разработчик может "подключиться" к этому моменту со своим кодом.

### **Преимущества использования событий:**
- ✅ **Не нарушаем ядро** - обновления системы безопасны
- ✅ **Модульность** - код в одном месте
- ✅ **Переиспользуемость** - один обработчик для разных задач
- ✅ **Безопасность** - стандартизированный подход

---

## **2. Типы событий в Битрикс**

### **2.1. События модулей (OnBefore, OnAfter)**
Срабатывают при вызове API-методов.

```php
<?
// Примеры событий для инфоблоков
...
```php
<?
// Агент, выполняемый каждые 10 минут
\Bitrix\Main\Config\Option::set("main", "agent_interval", 600);
?>
```

### **2.3. Почтовые события**
Обработка отправки email-сообщений.

---

## **3. Механика работы с событиями**

### **3.1. Регистрация обработчиков событий**

Основное место для регистрации - файл **`/local/php_interface/init.php`**
...
// Проверяем, что Битрикс загружен
if (defined('B_PROLOG_INCLUDED') && B_PROLOG_INCLUDED === true) {
    ...
    // 1. После добавления элемента инфоблока
    $eventManager->addEventHandler(
        'iblock',                       // модуль
        'OnAfterIBlockElementAdd',      // событие
        'myOnAfterElementAdd'           // функция-обработчик
    );
    
    // 2. Перед обновлением элемента инфоблока  
    $eventManager->addEventHandler(
        'iblock',
        'OnBeforeIBlockElementUpdate',
        'myOnBeforeElementUpdate'
    );
    
    // 3. После добавления заказа
    $eventManager->addEventHandler(
        'sale',
        'OnSaleOrderSaved', 
        'myOnOrderSaved'
...
## **4. Практические примеры обработчиков**

### **Пример 1: Автоматическая генерация символьного кода**
...
/**
 * Генерирует символьный код из названия, если он не заполнен
 */
function generateElementCode(&$arFields) {
    ...
    // Если код не заполнен и есть название
    if (empty($arFields['CODE']) && !empty($arFields['NAME'])) {
        
        // Транслитерация названия в код
        $code = CUtil::translit(
            $arFields['NAME'],
            'ru',
            [
        ...
        // Делаем код уникальным
        $arFilter = [
            'IBLOCK_ID' => $arFields['IBLOCK_ID'],
            'CODE' => $code
        ];
        
        if (CIBlockElement::GetList([], $arFilter)->Fetch()) {
            $code .= '-' . rand(100, 999);
        }
...
### **Пример 2: Валидация данных перед сохранением**
 ...
 */
function validateProductData(&$arFields) {
    ...
    if (empty($arFields['NAME'])) {
        $errors[] = 'Не заполнено название товара';
    }
    
    // Проверка цены через свойства
    $price = $arFields['PROPERTY_VALUES']['PRICE'] ?? null;
    if (empty($price) || $price <= 0) {
        $errors[] = 'Цена должна быть больше 0';
    }
    
    // Проверка артикула на уникальность
    $article = $arFields['PROPERTY_VALUES']['ARTICLE'] ?? '';
    if ($article) {
        $res = CIBlockElement::GetList(
            [],
            [
                'IBLOCK_ID' => $arFields['IBLOCK_ID'],
                '!ID' => $arFields['ID'] ?? 0, // исключаем текущий элемент при обновлении
    ...
    // Если есть ошибки - отменяем сохранение
    if (!empty($errors)) {
        global $APPLICATION;
...
### **Пример 3: Отправка уведомлений после действий**
...
$eventManager->addEventHandler(
    'iblock',
    'OnAfterIBlockElementAdd',
    'notifyAboutNewElement'
);

/**
 * Уведомление о новом элементе
 */
function notifyAboutNewElement(&$arFields) {
    ...
    if ($element = $res->Fetch()) {
        ...
        $eventFields = [
            ...
            'LINK' => 'https://site.ru/news/' . $element['CODE'] . '/'
        ];
        
        CEvent::Send('NEW_NEWS_NOTIFICATION', 's1', $eventFields);
        
        // Отправка в Telegram
        sendTelegramNotification(
            "📰 Добавлена новость: " . $element['NAME'] . 
            "\n🔗 Ссылка: https://site.ru/news/" . $element['CODE'] . "/"
        );
    }
}
...
### **Пример 4: Автоматическая обработка изображений**
...
/**
 * Автоматическая обработка изображений элемента
 */
function processElementImages(&$arFields) {
    ...
    if ($arFields['DETAIL_PICTURE']) {
        createImageThumbnails($arFields['DETAIL_PICTURE']);
    }
    
    // Обработка свойств-файлов
    $propertyValues = CIBlockElement::GetProperty(
        $arFields['IBLOCK_ID'],
        $elementId
    );
    
    while ($prop = $propertyValues->Fetch()) {
        if ($prop['PROPERTY_TYPE'] == 'F' && $prop['VALUE']) {
            createImageThumbnails($prop['VALUE']);
        }
    }
}

/**
 * Создание превьюшек изображения
 */
function createImageThumbnails($fileId) {
    $fileArray = CFile::GetFileArray($fileId);
    
    if (!$fileArray || !strpos($fileArray['CONTENT_TYPE'], 'image/') === 0) {
        return;
    }
    ...
    // Можно сохранить куда-то путь к превью
...
## **5. Работа со сложными событиями**

### **Пример 5: Обработка заказов интернет-магазина**
 ...
 */
function handleOrderChanges(\Bitrix\Main\Event $event) {
 ...
 */
function notifyAboutNewOrder($order) {
    ...
    $eventFields = [
        ...
        'LINK' => '/bitrix/admin/sale_order_view.php?ID=' . $orderId
    ];
    
    CEvent::Send('NEW_ORDER_NOTIFICATION', 's1', $eventFields);
    ...
    CEventLog::Add([
        ...
        'MODULE_ID' => 'sale',
        'DESCRIPTION' => "Создан заказ №{$orderId} на сумму {$price} руб."
    ]);
}
 ...
 */
function reserveOrderItems($order) {
...
## **6. Отладка и мониторинг событий**
 ...
 */
function logElementActivity(&$arFields) {
    
    $action = (isset($arFields['RESULT']) && $arFields['RESULT']) ? 'UPDATE' : 'CREATE';
    
    CEventLog::Add([
        'SEVERITY' => 'INFO',
        'AUDIT_TYPE_ID' => 'IBLOCK_ELEMENT_' . $action,
        'MODULE_ID' => 'iblock',
        'DESCRIPTION' => sprintf(
            "Элемент [%s] %s. Инфоблок: %s, Пользователь: %s",
            $arFields['ID'],
            $action == 'CREATE' ? 'создан' : 'обновлен',
            $arFields['IBLOCK_ID'],
            $GLOBALS['USER']->GetID()
        )
    ]);
...
/**
 * Функция для отладки - логирование в файл
 */
function debugEvent($data, $eventName = '') {
    $logFile = $_SERVER['DOCUMENT_ROOT'] . '/upload/events_debug.log';
    $timestamp = date('Y-m-d H:i:s');
    
    $message = "[{$timestamp}] Event: {$eventName}\n";
    $message .= "Data: " . print_r($data, true) . "\n";
    $message .= "User: " . $GLOBALS['USER']->GetID() . "\n";
    $message .= "=====================================\n";
...
```php
<?
// Файл для отладки - просмотр всех событий
function debugRegisteredEvents() {
    $eventManager = \Bitrix\Main\EventManager::getInstance();
    
    // Через рефлексию получаем список событий
    $reflection = new ReflectionClass($eventManager);
    $property = $reflection->getProperty('handlers');
    $property->setAccessible(true);
    
    $handlers = $property->getValue($eventManager);
...
```php
<?
// ПЛОХО - тяжелые операции в часто вызываемых событиях
$eventManager->addEventHandler(
    'main',
    'OnProlog',
    'heavyOperation' // вызывается на КАЖДОЙ странице
);

// ХОРОШО - оптимизированные обработчики
$eventManager->addEventHandler(
    'iblock',
    'OnAfterIBlockElementAdd', // вызывается редко
    'optimizedHandler'
);
?>
```
...
function safeEventHandler(&$arFields) {
    ...
    // Валидация ID
    if (isset($arFields['ID']) && !is_numeric($arFields['ID'])) {
        return;
    }
...
### **7.3. Организация кода:**
```php
<?
// Выносим обработчики в отдельные классы

class ProductEvents {
    public static function onAfterAdd(&$arFields) {
        if ($arFields['IBLOCK_ID'] == PRODUCTS_IBLOCK_ID) {
            self::generateProductCode($arFields);
            self::notifyManagers($arFields);
        }
    }
    
    private static function generateProductCode(&$arFields) {
        // логика генерации кода
    }
    
    private static function notifyManagers(&$arFields) {
        // логика уведомлений
    }
}
...
$eventManager->addEventHandler(
    'iblock',
    'OnAfterIBlockElementAdd',
    ['ProductEvents', 'onAfterAdd']
);
?>
```

**Ключевые моменты:**
1. **Всегда используйте `/local/php_interface/init.php`** для регистрации
2. **Проверяйте условия** перед выполнением кода в обработчике
3. **Обрабатывайте ошибки** - используйте try/catch
4. **Логируйте действия** для отладки
5. **Тестируйте обработчики** на всех сценариях
6. **Документируйте** свои обработчики

Этот подход позволяет создавать мощные расширения функциональности Битрикс, оставаясь в рамках поддерживаемой архитектуры.